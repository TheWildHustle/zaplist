import { SimplePool, nip19, nip47 } from 'https://esm.sh/nostr-tools@1.17.0'

const pool = new SimplePool()

let loggedInUser = null
let zapSendersResults = null
let nwcConnected = false
let nwcPublicKey = null
...
async function fetchZapSenders() {
  ...
  let myPubkey = nwcConnected ? nwcPublicKey : pubkeyInput.value.trim()
...
async function connectNWC() {
  try {
    const uriString = prompt("Please enter your NWC URI:");
    if (!uriString) return;

    const nwcConnection = nip47.makeConnection(uriString);
    const info = await nwcConnection.getInfo();

    nwcConnected = true;
    nwcPublicKey = info.pubkey;

    updateNWCStatus();
    fetchUserProfile(nwcPublicKey);
  } catch (error) {
    console.error("NWC connection error:", error);
    alert("Failed to connect to NWC. Please try again.");
  }
}
...
function updateNWCStatus() {
  ...
  if (nwcConnected) {
    nwcStatus.textContent = `Connected: ${nip19.npubEncode(nwcPublicKey)}`;
    nwcStatus.style.display = 'block';
    disconnectWalletBtn.style.display = 'inline-block';
    pubkeyInputContainer.style.display = 'none';
  } else {
    nwcStatus.style.display = 'none';
    disconnectWalletBtn.style.display = 'none';
    pubkeyInputContainer.style.display = 'block';
  }
}
...
document.getElementById('nwcBtn').addEventListener('click', connectNWC)
